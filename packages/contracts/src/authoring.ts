...
export const ReferenceImageUploadSchema = z.strictObject({ recordingId: IdSchema, recordingHash: HashSchema, frameIndex: RevisionSchema.max(3599), source: z.enum(['quest-camera', 'workspace-webcam']), image: VisionImageSchema });
export type ReferenceImageUpload = z.infer<typeof ReferenceImageUploadSchema>;

import { GuideEventSchema } from './guide.js';
export const SpectatorStateSchema = z.strictObject({ type: z.literal('spectator-state'), connected: z.boolean(), updatedAt: z.number().nonnegative(), ageMs: z.number().nonnegative(), snapshot: GuideEventSchema.nullable(), step: z.strictObject({ title: z.string(), instruction: z.string(), index: z.number().int().min(1), total: z.number().int().min(1), completionMode: z.string(), source: z.string() }).nullable() });
...
// Raw UTF-8 recording chunks preserve exact cross-language hashes; decoded JSON still uses RecordingSchema.
export const RecordingByteChunkSchema = z.strictObject({ dataBase64: z.string().min(1).max(2_796_204).regex(/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/), sha256: HashSchema });
