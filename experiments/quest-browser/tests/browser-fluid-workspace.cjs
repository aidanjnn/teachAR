...
(async()=>{const browser=await chromium.launch({headless:true,args:['--enable-unsafe-swiftshader','--mute-audio']});try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  const scene=new THREE.Scene(),g=new TutorialGuide({speak:()=>{},exit:()=>{}});await g.restore();g.attach(scene);await Promise.all([g.skinHand.loaded,g.leftGhost.skinHand.loaded]);g.sample=()=>data[g.hand];g.begin('home');g.action('create');if(!g.fluidCapture||g.mode!=='setup-new')fail('Default is not fluid in AR');
  g.action('setup-ready');g.start=[0,1,0];g.end=[.5,1,0];g.setWorkspace();g.action('placement-ready');if(g.mode!=='author')fail('Fluid capture unnecessarily requires a return-save position');
  ...
  for(let i=1;i<=30;i++){data=pair(.2*i/30);tick();}tick(40);await Promise.all([...g.segmentJobs]);await g.saveQueue;
  if(g.mode!=='capture'||g.tutorial.steps.length!==1||g.tutorial.steps[0].reviewed||g.tutorial.steps[0].acceptance!=='hold')fail('First hold did not continue capture with honest acceptance');
  tick(70);if(g.tutorial.steps.length!==1)fail('Resting hands repeatedly saved');
  for(let i=1;i<=30;i++){data=pair(.2+.2*i/30);tick();}tick(40);await Promise.all([...g.segmentJobs]);await g.saveQueue;
  if(g.tutorial.steps.length!==2||g.mode!=='capture')fail('Second segment not saved continuously');
  g.action('fluid-stop');await g.saveTask;const exported=validateTutorial(g.exportData());if(!learningReadiness(exported).ready||g.mode!=='saved'||exported.steps.length!==2)fail('Finish did not produce ready tutorial without another idle step');
  ...
  const timerSource={},q=new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1,0,0),-Math.PI/2),p=spatial.timer.position.clone();
  const above={transform:{position:{x:p.x,y:p.y+1,z:p.z},orientation:q}};
  ...
  let resolveMedia;const media=new TutorialGuide({speak:()=>{},exit:()=>{},media:{enable:()=>new Promise(r=>resolveMedia=r),cancel:()=>{}}});media.attach(new THREE.Scene());media.persist=()=>Promise.resolve();media.begin('home');media.action('create');media.action('media-enable');media.hide();resolveMedia({camera:true,microphone:true,errors:[]});await media.mediaTask;if(media.mode!=='setup-new')fail('System permission blur stranded setup');
