...
test('stationary midpoint palms cannot complete the reviewed 18 cm movement',()=>{
 const f=new TutorialFollower(linearStep()),d=driver(f);
...
test('each moving required hand must progress, even inside overlapping targets',()=>{
 const f=new TutorialFollower(linearStep()),d=driver(f);
 d.hold(hands(0));
...
test('wrong-direction, sideways and small oscillating motion cannot satisfy a gate',()=>{
 ...
 for(const [start,pose] of [[0,hands(-.005)],[.05,sideways]]){
  const f=new TutorialFollower(linearStep()),d=driver(f);d.hold(hands(start));d.hold(pose,5000);
  assert.equal(f.index,1);assert.equal(f.done,false);
 }
 const f=new TutorialFollower(linearStep()),d=driver(f);d.hold(hands(0));
 for(let i=0;i<125;i++)d.tick(hands(i%2?.01:0));
 assert.equal(f.index,1);assert.equal(f.done,false);
});

test('short movements still require observed motion; static references only require a hold',()=>{
 const f=new TutorialFollower(linearStep(.03)),d=driver(f);d.hold(hands(.015),5000);
 assert.equal(f.done,false);d.hold(hands(0));d.hold(hands(.03));assert.equal(f.done,true);
 const still=new TutorialFollower(linearStep(0));driver(still).hold(hands(0),2000);assert.equal(still.done,true);
});

test('fresh ordered motion completes with one or both required hands',()=>{
 for(const choice of ['left','right','both']){
  const f=new TutorialFollower(linearStep(.18,choice)),d=driver(f);
...
test('a stationary support hand stays required without inventing motion for it',()=>{
 const s=linearStep();for(const frame of s.frames)frame.right=hand(.4);
 const f=new TutorialFollower(s),d=driver(f);d.hold(hands(0));
 for(const x of [.09,.18])d.hold({left:hand(x),right:hand(.4)});
 assert.equal(f.done,true);
});

test('interruptions discard partial movement and do not credit a hidden jump',()=>{
 const interruptions=[
  (f,d)=>f.pause(),
  (f,d)=>d.tick({}),
  (f,d)=>d.stall(),
 ...
 for(const interrupt of interruptions){
  const f=new TutorialFollower(linearStep()),d=driver(f);d.hold(hands(0));d.tick(hands(.03));
