...
export function createRuntimeObserver({telemetry,guide,now=()=>performance.now(),timeoutMs=5000,maxPending=8}={}) {
  ...
  const signature=()=>JSON.stringify([mode(),guide.player?.index??-1,guide.tutorial?.revision??0,
    ...
    guide.player?.rate,guide.followEngine?.state,guide.followEngine?.index,objectId(guide.followEngine),
    guide.libraryIndex,guide.cleanSave,guide.alignmentEnabled,guide.problem||'',guide.note||'',guide.savedMessage||'']);
  function context() {
    ...
    const practice=['learn','learn-options'].includes(mode());
  ...
  function stage(item,name,extra={}) {
    if(item?.id)emit('interaction',{interaction_id:item.id,stage:name,control:item.control||'none',source:item.source,
  ...
  function acceptedAction(item,before,after) {
    ...
    const control=item.control,wasPractice=['learn','learn-options'].includes(before.mode);
    const becamePractice=after.mode==='learn'&&!wasPractice;
    let action=null;
    if(control==='primary'&&before.mode==='learn'&&before.done&&!before.watchOnly&&(after.mode==='finished'||after.step!==before.step))action='confirm';
    else if(wasPractice&&['restart-follow','try-follow','verify'].includes(control)&&before.signature!==after.signature)action='repeat';
    else if(control==='watch-demo'&&before.mode==='learn'&&!before.watchOnly&&guide.watchOnly)action='help';
    ...
    else if(control==='learn-options'&&before.mode==='learn'&&after.mode==='learn-options')action='pause';
  ...
  function dispatch(interactionId,fn) {
    ...
    safe(()=>{
      ...
      before={signature:signature(),context:context(),mode:mode(),step:guide.player?.index??-1,
        done:!!guide.followEngine?.done,watchOnly:!!guide.watchOnly,paused:!!guide.gatePaused};
  ...
  function observe(options={}) {
    ...
    const info=context(),practice=['learn','learn-options'].includes(mode());
    ...
    if(practice&&active&&!visible)phase='hidden';
      ...
      if(mode()==='learn-options'||guide.gatePaused||guide.watchOnly)phase='user_paused';
      else if(!fresh)phase='system_wait';
      else if(guide.pending||guide.followEngine?.invalid||(!requiredLeft&&!requiredRight))phase='system_wait';
