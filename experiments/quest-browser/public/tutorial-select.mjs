...
export function enhanceSelects(root=document){
 for(const select of root.querySelectorAll('select')){
  ...
  const paint=()=>{list.replaceChildren();options().forEach((option,i)=>{const row=document.createElement('div');row.id=`${list.id}-${i}`;row.setAttribute('role','option');row.setAttribute('aria-selected',String(option.selected));row.setAttribute('aria-disabled',String(option.disabled));row.textContent=option.textContent+(option.selected?' ✓':'');row.dataset.highlight=String(i===cursor);row.onmousedown=e=>e.preventDefault();row.onclick=e=>{e.preventDefault();choose(i);};list.append(row);});trigger.setAttribute('aria-activedescendant',`${list.id}-${cursor}`);};
