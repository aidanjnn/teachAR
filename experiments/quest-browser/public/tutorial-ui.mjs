...
export function tutorialView(g){
 ...
 case 'learn':{
  ...
  if(g.coach?.active&&g.coach.caption&&!g.watchOnly&&state!=='checkpoint')v.detail=`Coach: ${g.coach.caption.slice(0,150)}`;
  b('replay',g.watchOnly?(g.player.paused?'Resume replay':'Pause replay'):(g.gatePaused?'Resume':'Pause'));b('watch-demo','Watch again');if(g.coach?.active)b('coach-ask',g.coach.mode==='listening'?'Listening…':'Ask coach');b('learn-options','More options');break;}
 ...
 if(g.problem){v.text=g.problem;}
 if(g.saveStatus==='saving')v.detail=g.savedMessage;
 if(g.saveStatus==='failed'){v.title='Could not save locally';v.text=g.savedMessage;v.detail='Your changes are still open here.';if(v.buttons.length<6)b('retry-save','Retry local save');}
 return v;
}
