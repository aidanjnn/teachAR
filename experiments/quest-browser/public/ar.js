...
window.addEventListener('beforeunload',event=>{if(tutorialMode&&(['capture','capture-paused','confirm-home','confirm-exit','confirm-discard','saving','saving-tutorial'].includes(guide.mode)||guide.mode==='settings'&&['capture','capture-paused'].includes(guide.settingsReturn)||['saving','failed'].includes(guide.saveStatus))){event.preventDefault();event.returnValue='';}});
