...
export function mountReview(guide,{isActive,tell}){
  ...
  function renderList(){
    ...
    steps.forEach((step,i)=>{
      ...
      button.textContent=`${i+1}. ${step.instruction||'Untitled'} · ${(step.duration_ms/1000).toFixed(1)}s · ${step.reviewed?'reviewed':step.acceptance?'accepted during capture':'needs review'}`;
  ...
  $('save-step-edits').onclick=()=>void report(async()=>{
    ...
    step.guide_hands=$('guide-hands').value;step.instruction=$('step-instruction').value;step.reviewed=$('reviewed').checked;step.acceptance=null;
