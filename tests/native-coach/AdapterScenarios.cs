...
internal static class AdapterScenarios
{
    ...
    private static void SynchronousExpiryReleasesResources(bool duringContextUpdate)
    {
        ...
        if (!duringContextUpdate)
            typeof(NativeVoiceCoach).GetMethod("Raise", BindingFlags.NonPublic | BindingFlags.Instance)
                .Invoke(coach, new object[] { CoachEvent.Of(CoachEventKind.ListenToggled) }); // Must be discarded by invalidation.
        Tick(coach);
        if (duringContextUpdate)
        {
            coach.Connection.Pending(201, "{\"schemaVersion\":1,\"sessionId\":\"session-1\",\"sdp\":\"v=0\",\"liveModel\":\"mock\"}");
            Tick(coach);
            Check(coach.Session.State.Mode == CoachMode.Listening, "explicit Start establishes hands-free listening after readiness");
            Check(mic.Capturing, "capture is enabled before context update");
