...
- **Catch-up:** `main` moved to `dfaf572` (PR #30 landed the immersive launcher) during the review. Fast-forwarded and reapplied; the Voice coach card now lives under **Browser tools · review, import and backup** with its hooks intact, and `docs/voice-demo-checklist.md` describes that path. Full gate after the catch-up: `pnpm check`, fixtures, 8 e2e flows and the WebXR suite green.
