...
### 2026-09-20 — Ignore recorded support-hand jitter in PR #26 guidance

- The babysitting heartbeat verified Greptile's P1 against head `fe4444b`: every nonzero support-hand gate displacement required matching live directional movement, even when the recorded hand only contained small tracking variations. New deterministic three-axis jitter regressions failed in both ordered and relaxed guidance before the repair.
- Classify a required hand as stationary only when its entire recorded path fits within a 1 cm bounding-box diagonal. Such hands still require fresh tracking and target proximity at every gate, without requiring imitation of arbitrary jitter. Hands with larger recorded spans retain the existing directional motion fraction; positional radii, dwell, loss/recovery behavior and serialized formats are unchanged. Documented the threshold as provisional, with deliberate sub-centimetre actions outside movement validation; it is not a measured Quest noise bound.
- Fresh `pnpm setup:webxr` and `pnpm test:webxr` passed: **88 Node tests, 53 Python tests and all 11 synthetic browser workflows**. Added coverage for noisy support, missing/off-target support and deliberate 3 cm secondary-hand motion in both modes; existing stationary-bypass, wrong-direction and interruption regressions remain green. Reused the unchanged workspace/shared/backend/desktop/fixture inputs from passing hosted run `35492561903`. Patch whitespace and README targets passed. Provider credentials were disabled and browser data temporary; no new headset, live-provider or physical-transfer evidence. No reviewer reply, thread resolution, merge or deployment was performed.
