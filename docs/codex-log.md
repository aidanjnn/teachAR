...
- **Aidan's staff review of PR #36 (posted after merge, 73/100), fixed on this branch:** (1) after an origin reset during a take, discarding the take now routes through placement (`setup-new`) and Start recording refuses a stale registration the same way, so no new samples are transformed with the pre-reset workspace; (2) a delegated voice action is bound to the learner's utterance context, captured at their last transcript words, not to the screen showing when the backend's delegation arrives, so "next step" followed by a manual Next no longer advances twice (regression with Aidan's event order); (3) Repeat & practise lets the first generated instruction finish before the loop restarts and goes silent, matching Guided movement. Browser regressions added to `browser-simple-create.cjs` and `browser-workspace-experience.cjs`; web coach test added.
- **Review round (PR #37):** the automated review returned 13 findings; 11 fixed, 2 deferred. Fixed: a fragmented question's tail can no longer act (an unfinished fragment is judged together with the next one; a finished question is not), a one-phrase question ("Next step?") never matches, the coach's speech flushes a pending command instead of dropping it, dedup works in both directions (tool first then lagging transcript, and local first then tool echo) and a new learner utterance ends the echo window, single ordinary words are no longer phrases, guide failures inside the local path are caught and reported, a long turn stays quiet until the learner pauses, dead "now" phrases removed, one gate for active/visible, tests exercise the stutter guard with a three-step tutorial and a scheduler that forgets fired timers, and the prompt no longer forbids delegation (the fallback stays; the model is only asked not to narrate). Deferred: sharing one phrase table with the server's clip-pipeline fast path in `voice-intent.ts` (that path is diagnostics-only now) and the doc wording about a reset on screen change (corrected in code comments instead).
