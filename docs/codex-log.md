...
### 2026-09-20 — OMNI Live track research

- **Request / source:** User asked for read-only research on entering the Huawei OMNI Live track with Trail, later adding a teammate's object-anchoring proposal to reconcile. Track README from the user's download; gateway facts from yibuapi's public pricing/status JSON and Apifox docs; codebase inventory from `main` after PR #32.
- **Findings:** the track wants a demo-ready app using an OMNI model (Qwen3.5-Omni via the yibuapi OpenAI-compatible gateway) with vision, speech and language working together in one scenario; Trail has the camera snapshot module, per-step reference photos, the paired API and an unconnected vision service, but nothing reads the `OMNI_*` placeholders and no key exists yet. Minimal honest build: one grounded route sending a fresh frame plus the step's reference photo plus a spoken question to `qwen3.5-omni-flash`, speech back, advice only. The object-anchoring flow (marker mat, corner refinement, versioned anchor metadata) is the stated next step, not today's build.
- **Output:** `docs/omni-live-track-research.md` on `codex/omni-scene-coach`. No code changed; no provider calls made.

### 2026-09-20 — OMNI scene coach: Look & advise

- **Request / source:** User asked to implement the minimal OMNI plan from `docs/omni-live-track-research.md` on `codex/omni-scene-coach`, keeping the open PR #31 structure in mind. Design: `docs/superpowers/specs/2026-09-20-omni-scene-coach-design.md`.
- **Server:** `SCENE_COACH`, `OMNI_API_KEY`, `OMNI_BASE_URL`, `OMNI_MODEL`, `OMNI_VOICE` in `config.ts` (off by default, key required when on); `ai/scene-coach.ts` with the `SceneCoachProvider` interface, the Qwen-Omni gateway (one streamed chat completion with `modalities: ["text","audio"]`, SSE and single-JSON parsing, PCM16 24 kHz wrapped as WAV, bounded body, status-only errors) and the lexical guard ported from the vision service; `routes/scene-coach.ts` behind the pairing guard with `groundContext`, sharp bounds (JPEG only, re-encoded within 1024 px), 3 s freshness, one in flight, 3 s spacing, 60 per process, 15 s deadline; `createApp({ sceneCoach })` for tests; `providers.scene` in health (`HealthSchema` extended). Contracts: `SceneAdviceRequestSchema`, `SceneAdviceResponseSchema`.
- **Tutor:** `public/scene-coach.mjs` (fresh frame via the existing snapshot, reference photo, grounded context from the coach adapter's new `contextFor`, Web Audio playback with a stale-answer watchdog, tutor voice fallback, captions through `announce`); Look & advise on the practice panel and the Voice coach card, shown only when the coach is active and a camera stream is live; the AR page holds its own narration while advice plays; Stop, leave and session end cancel it.
- **Automated evidence:** gateway parser, WAV wrapping, guard and error mapping unit tests; route tests for off, bounds, grounding, guard, spacing and 503 without details; tutor Node tests for request shaping, refusals, stale drop and cancellation; `browser-coach.cjs` drives the real page with the fake camera through a mocked route and checks that the advice is grounded on the published guide and changes nothing in the tutorial.
- **Catch-up:** `main` moved to `26c2162` (PR #33 Sentry telemetry, PR #34 OpenAI track brief) while this was built; merged with six small conflicts (env example, app and config wiring, the AR page's pagehide and preview handlers, the Python allowlist, this log), all resolved by keeping both sides. Health now carries `providers.scene` next to the telemetry config.
- **Review round (PR #35):** the automated review returned 15 findings; 13 fixed (suspended-AudioContext hang, generation-guarded playback state, Stop coach and tutorial change stopping the scene coach, a paid model refused on an unpaired server, bounded single-JSON bodies and cancelled error bodies, a per-look abort controller, the tutor-voice fallback counted as speaking, off-check before body parsing, parallel image bounding, compact-row button cap, busy status precedence, useful server logs, dead code and doc slips), 2 deferred (a guard and sharp helper shared with the retired vision service; caching the bounded reference photo).
- **Staff review (PR #35, Aidan, 74/100, changes requested):** three findings, all fixed with regressions. The gateway now returns the complete transcript and the route judges every sentence before applying the 600-character caption cap; a claim after the cut, or an answer longer than the cap, is guarded and its audio dropped. The browser freshness check includes the coach's run and attempt identity and its activity, so Repeat, a restarted coach or a stopped coach before the answer arrives drops it. The single-JSON gateway path counts bytes while reading and cancels at 8 MiB before decoding. Measured overheads for the report: sharp bounding 1.7 ms for a 640x480 frame and 4.8 ms for a 1280x960 reference photo; the browser workflow prints the press-to-caption round trip with the model mocked.
- **Not verified:** no yibuapi key existed during this work, so no real OMNI call was made; `scripts/omni-smoke.mjs` is the first thing to run when the key arrives. Camera plus immersive session concurrency on the Quest 3S remains a headset gate; the flat tutor page and the laptop webcam are the fallback demo paths.
