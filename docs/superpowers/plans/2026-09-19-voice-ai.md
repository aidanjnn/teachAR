...
- Produces: `registerVoiceRoutes(app, provider)`, `createApp(config, { webRoot?, logger?, provider? })`. Routes: `POST /api/voice/transcriptions`, `POST /api/voice/labels`, `POST /api/coach`, `POST /api/coach/session`.
...
Expected: `apps/web/dist/voice-lab.html` exists. Then `pnpm dev` and open `http://localhost:5173/voice-lab.html`; record, stop, play, transcribe (fixture spans appear), generate labels (fallback), connect coach (text), ask.
