...
The tutor works independently of the TypeScript backend. Connecting approved
browser steps to paired backend services requires an explicit adapter; no hidden
protocol compatibility is assumed. The headset browser is the sole progression
...
1. **Coach adapter:** connect approved browser context to paired voice/vision
   services with one microphone owner, lifecycle cleanup, generations, opt-in
   capture, budgets and cancellation. Voice Lab is currently separate.
