...
| Voice coach | Main repository has a separate Voice Lab and backend adapters | Not connected end to end to this tutor's tutorial format/context; needs deliberate integration |
...
Browser `trail.tutorial.prototype.v3` is not the shared API's recording/tutorial v1 contract. A coach adapter must build validated approved-step context and explicit generation IDs; never silently submit browser JSON as an API recording. Keep provider keys on the server. The headset browser alone owns its local learner progression; AI/desktop observations cannot advance it.
...
1. **One coherent coach integration.** Bridge approved browser step/instruction/
   reference/current-frame context into the existing paired backend. Add mic
   ownership/teardown, opt-in voice, stale step/attempt rejection, budgets and
   cancellation. Integrate OMNI only after API/access is confirmed.
