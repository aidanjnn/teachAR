...
3. **One coherent coach integration.** Bridge approved step/instruction/reference/current-frame context into the existing paired backend. Add mic ownership/teardown, opt-in voice, stale step/attempt response rejection, budgets and cancellation. Integrate OMNI behind a replaceable adapter only after its actual API/access is confirmed. Do not run two competing voice loops.
