...
- [ ] **U1 — One Create/Follow shell** (TRAIL-08/12). Coordinate existing Capture, Guide and Storage controllers through a single experience controller. Home offers Create tutorial, Follow tutorial, Library and Settings; diagnostics stay in Settings. Standardize button hit areas, dwell/withdraw behavior, back/cancel and status placement. **Pass:** a new learner can reach follow mode without the laptop or contradictory engineering panels; author/learner pairing failures have a clear recovery path.
  - *Implementation (branch `codex/native-mvp-integration`):* added `Trail.Shell` (`Runtime/Shell/`) — a new assembly, because `Trail.Presentation` cannot reference `Trail.Guide` without a cycle. `ShellModel` routes Home/Create/Follow/Library/Settings with role gating and per-control reasons; `ShellInteraction` gives every panel one touch→hold→withdraw confirm rule, replacing the split where capture confirmed on withdrawal but guide and storage fired on dwell. `TutorialExperienceController` is a thin adapter that only delegates. `IDiagnosticPanel` lets the shell hide all four engineering panels outside Settings. **Evidence:** 195 checks in `tests/native-shell/` against the real `ShellModel`/`ShellInteraction` sources. **Not evidence of:** Unity compilation, world-space layout, legibility, or that a learner can use it. `TutorialExperienceController` has never been compiled.
- [ ] **U2 — Set the save position once** (TRAIL-08). Create begins with stable visible left/right palm positions; after workspace registration persist them in workspace coordinates. Every take reuses these positions. Re-record, pause, review and next-step must not silently recapture them. Provide explicit Change save position; clear it for a new tutorial. **Pass:** record two different actions, returning to the same spot after each; inspect saved clips and metadata. Missing tracking, ordinary return-like task motion and accidental dwell must not save a bogus step.
  - *Implementation:* `Motion/SaveZone.cs`, `RecordingState.cs`, `RecordingDirector.cs`, `TakeLedger.cs` — a pure reducer, so it is genuinely compiled and tested. **Evidence:** `tests/native-capture` now runs a save-position/take-lifecycle suite (`Tests/EditMode/RecordingFixtureAssertions.cs`, 139 assertions in 12 groups) plus a mutation pass in which 14 deliberate defects were each caught. **Not evidence of:** anything Unity or on-device. Every threshold (800 ms holds, 8/10/15 cm bands, 3 s countdowns, 6 s candidate expiry) is ported from the browser reference, not measured on a headset. Not yet wired into `CaptureReplaySession`.
- [ ] **U3 — Clean recording lifecycle** (TRAIL-04/08). Port pause/resume and deliberate endpoint-hold → return-to-save detection as testable C# logic. Trim poses and narration on the same timeline; exclude the return gesture and UI reaching. Preserve the previous take if its replacement is discarded. **Pass:** replay begins/ends on the demonstrated action, not button presses; pause, tracking loss, origin invalidation and cancel reset dwell; checkpoint time cannot include paused time. Keep explicit Stop available when a task naturally returns to the save position.
  - *Implementation:* same reducer. Endpoint-hold → return-to-save, pause/resume, and one trim boundary `[StartMs, EndMsExclusive)` applied identically to frames, markers and narration. The take clock advances only from fresh sample deltas while recording, so arming, pauses and stalls are excluded from both hold timing and the saved timeline. **Known ported limitation:** the endpoint gesture arms as soon as the hands leave the save zone, so holding still away from home early in a take creates a candidate; guarded only by the 6 s expiry, as in the reference. Explicit `StopFullTake` remains available and tested. **Not implemented here:** the actual PCM/Opus clipping — the pure layer emits the interval; an adapter must cut the bytes.
- [ ] **U4 — Paired ghost and honest green zones** (TRAIL-07/12). `GhostPresentation` currently selects one hand and `GuideController` presents `Targets[0]`. Render both required hands, with clear translucent expert geometry and distinct live-hand outlines. Draw the actual reviewed tolerance volume, not a fixed decorative radius. **Pass:** both hands remain legible against light/dark tables, missing tracking appears neutral, and green means motion proximity—not verified object placement. Tune on a real headset; do not claim centimeter accuracy from the configured radius.
  - *Implementation:* `GhostPresentation` now builds one rig per side (both eager in `Awake`, both hidden) and `ShowGuideHands(frame, hands)` replaces the single-hand `ShowGuideFrame`, which is deleted so the `Targets[0]` bug cannot be rewritten. The tolerance zone is drawn at the reviewed `PositionToleranceM`; out-of-contract radii draw nothing rather than inventing one. An untracked hand hides its own rig and zone, so missing tracking reads as absent, never pass or fail. **Not evidence of:** anything visual. Legibility of two translucent teal rigs on light/dark tables, whether the zone reads as a volume, and frame-time for 104 primitives on a Quest 3S are all unmeasured. Left and right share one material and are not colour-differentiated. **No two-hand PlayMode test exists** — the core of U4 is natively untested.
- [ ] **U5 — Follow at the learner's pace** (TRAIL-06/07). Keep the native start/ordered-gate reducer; wire Watch → Your turn → Check result presentation to it. The ghost waits at the start until the required hands dwell there, then stays modestly ahead. Pause on missing required observations; require fresh reacquisition. **Pass:** wait 10 seconds before starting without the ghost running away, deliberately drift sideways and recover, skip a gate, cover a hand, repeat a step and resume after backend loss. No skipped gate or duplicate advancement. Recorded narration is not yet retimed to adaptive following; do not replay it misleadingly.
  - *Implementation:* `GuideController` loops over every `step.Targets` instead of `Targets[0]`, with per-target cue frames indexed by `state.CueProgress[i]`, and exposes `PhaseLabel` (Watch / Your turn / Check result) prefixed onto `Status`. `GuideReducer` is untouched and remains the sole progression authority; the bounded `+2` frame lookahead still never feeds observations or gates. **Not evidence of:** the device scenarios this item requires — waiting 10 s, drifting sideways, skipping a gate, covering a hand, repeating a step, resuming after backend loss. Recorded narration is still not retimed to adaptive following; do not replay it during guided practice.
