...
describe('durable authoring HTTP flow', () => {
  it('uploads, compiles, reviews, finalizes and reloads immutable steps across restart', { timeout: 20000 }, async () => {
    ...
    try {
      ...
      const learnerToken = (await s.app.inject({ method: 'POST', url: '/api/pair', headers: { host: s.headers.host }, payload: { code: s.auth.issueCode('learner').code, client: 'native' } })).json().token;
      const learner = { host: s.headers.host, authorization: `Bearer ${learnerToken}` };
      expect((await s.app.inject({ method: 'POST', url: `/api/tutorials/${id}/layout/query`, headers: learner })).statusCode).toBe(403);
      expect((await s.app.inject({ method: 'PUT', url: `/api/tutorials/${id}/layout`, headers: learner, payload: {} })).statusCode).toBe(403);
      expect((await s.app.inject({ method: 'POST', url: '/api/tutorials/ready/query', headers: learner })).json()).toEqual([]);
      expect((await s.app.inject({ method: 'POST', url: `/api/tutorials/${id}/layout/query` })).statusCode).toBeGreaterThanOrEqual(400);
      ...
      expect((await s.app.inject({ method: 'POST', url: `/api/tutorials/${id}/layout/query`, headers: learner })).statusCode).toBe(200);
      expect((await s.app.inject({ method: 'POST', url: '/api/tutorials/ready/query', headers: learner })).json()).toHaveLength(1);
