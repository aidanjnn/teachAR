...
namespace Trail.Runtime.Scene
{
    ...
    public sealed class PrivateExpertReferenceStore
    {
        ...
        private string PathFor(string id, string kind = "endpoint")
        {
            if (!Guid.TryParse(id, out var parsed)) throw new ArgumentException("Native take ID must be a UUID.");
            if (kind != "endpoint" && kind != "starting-layout") throw new ArgumentException("Unknown image kind.");
            return Path.Combine(directory, parsed.ToString("N") + (kind == "starting-layout" ? "-layout" : "") + ".json");
        }
        private static bool Finite(double value) => !double.IsNaN(value) && !double.IsInfinity(value);
        public void Save(Recording recording, ExpertReferenceCandidate candidate, int frameIndex, string kind = "endpoint")
        {
            ...
            if (kind == "starting-layout" && frameIndex != 0) throw new ArgumentException("Starting layout must depict the first frame.");
            var value = new PrivateExpertReference { kind = kind, recordingId = recording.Id,
            ...
            var path = PathFor(recording.Id, kind); var text = JsonUtility.ToJson(value);
        ...
        public PrivateExpertReference Load(string recordingId, string kind = "endpoint")
        {
            var path = PathFor(recordingId, kind);
            ...
            // Older private endpoint files predate the kind field; their original filename remains authoritative.
            if (value != null && string.IsNullOrEmpty(value.kind) && kind == "endpoint") value.kind = "endpoint";
            if (value == null || value.kind != kind || (kind == "starting-layout" && value.frameIndex != 0) || value.recordingId != recordingId || value.source != "quest-camera" || value.timing != "delivery-aligned-unverified" ||
