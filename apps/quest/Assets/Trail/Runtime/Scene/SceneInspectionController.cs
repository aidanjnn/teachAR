...
namespace Trail.Runtime.Scene
{
    ...
    public sealed class SceneInspectionController : MonoBehaviour
    {
        ...
        private string appSessionId = Guid.NewGuid().ToString();
        private int epoch;
            ...
            Connection.Request("POST", "/api/guide-events", ContractJson.SerializeGuideEvent(paused), (status, _) => {
                ...
                Connection.Request("POST", "/api/inspections", start, (startStatus, body) => {
                    ...
                    try {
                        ...
                        if (next.Request.LiveSessionId != appSessionId || next.Request.RequestEpoch != requestEpoch ||
        ...
        private void OnConnectionInvalidated() { Cancel(); appSessionId = Guid.NewGuid().ToString(); epoch = 0; Status = "Connection unavailable; guide remains paused until Resume."; }
