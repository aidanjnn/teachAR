...
const DURATION_KEYS = ['demo_preview_ms', 'ready_ms', 'following_ms', 'user_paused_ms', 'tracking_lost_ms', 'system_wait_ms', 'checkpoint_ms', 'unknown_ms'];
const COUNT_KEYS = ['tracking_interruptions', 'repeats', 'help_requests', 'confirmations', 'samples'];
const PHASE_KEYS = {demo_preview: 'demo_preview_ms', ready: 'ready_ms', following: 'following_ms', user_paused: 'user_paused_ms', tracking_lost: 'tracking_lost_ms', system_wait: 'system_wait_ms', checkpoint: 'checkpoint_ms'};
...
const phaseOf = data => {
  ...
  if (['idle', 'demo_preview', 'user_paused', 'system_wait', 'checkpoint'].includes(data.phase)) return data.phase;
...
export function createFrictionObserver({emit = () => {}, maxSteps = 60, maxGapMs = 2500} = {}) {
  ...
  const aggregate = summary => {
    ...
    else if (summary.outcome !== 'checkpoint') group.interrupted++;
  ...
  const consume = (record, {emit: shouldEmit = true} = {}) => {
    ...
    if (record.type === 'guide_state' && (data.phase === 'idle' || !validIdentity(data))) {
      advance(record.t_ms, data.observation_gap_ms); finish(active?.checkpoint_reached ? 'checkpoint' : 'left', shouldEmit); return;
    }
