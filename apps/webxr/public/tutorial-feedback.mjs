...
export class FeedbackAudio {
 ...
 play(event){
  ...
  const frequencies=event.kind==='saved'?[660,880]:event.kind==='error'?[220]:event.kind==='checkpoint'?[740,988]:event.kind==='open'?[520,780]:event.kind==='pause'?[480,360]:[440,620];
  frequencies.forEach((f,i)=>{const o=c.createOscillator(),g=c.createGain();o.frequency.setValueAtTime(f*.72,start+i*.1);o.frequency.exponentialRampToValueAtTime(f,start+i*.1+.045);o.type='sine';g.gain.setValueAtTime(0,start+i*.1);g.gain.linearRampToValueAtTime(.035,start+i*.1+.015);g.gain.exponentialRampToValueAtTime(.0001,start+i*.1+.13);o.connect(g);g.connect(c.destination);o.start(start+i*.1);o.stop(start+i*.1+.14);o.onended=()=>{o.disconnect();g.disconnect();};});
 }
