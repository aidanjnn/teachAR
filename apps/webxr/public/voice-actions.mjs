...
export function applyVoiceCommand(g,command){
 ...
 if(command==='finish'&&(capture||g.mode==='author')){
  if(capture&&g.fluidCapture)g.finishFluid();else if(g.mode==='author')g.saveTask=g.finishAuthoring();else return {ok:false,message:'Save the current step first.'};
 ...
 if(follow&&['replay','next','previous'].includes(command)){
  ...
  g.audioPlayer?.stop();g.coach?.onAttempt();g.epoch++;g.player.index=index;g.watchOnly=false;g.gatePaused=false;g.showStep();
