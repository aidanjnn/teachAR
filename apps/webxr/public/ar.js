...
window.addEventListener('beforeunload',event=>{if(tutorialMode&&(['capture','capture-paused','confirm-home','confirm-exit','confirm-discard','saving','saving-tutorial'].includes(guide.mode)||['settings','voice','voice-help'].includes(guide.mode)&&['capture','capture-paused'].includes(guide.mode.startsWith('voice')?guide.voiceReturn:guide.settingsReturn)||['saving','failed'].includes(guide.saveStatus))){event.preventDefault();event.returnValue='';}});
