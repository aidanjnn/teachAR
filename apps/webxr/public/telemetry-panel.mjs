...
const PHASE_LABELS = {idle: 'Idle', demo_preview: 'Automatic demonstration', ready: 'Waiting at the start', following: 'Following', user_paused: 'Paused by learner', tracking_lost: 'Required hand unavailable', system_wait: 'Application waiting', checkpoint: 'Movement checkpoint reached', hidden: 'View hidden · observation unknown'};
...
export function mountDiagnostics({telemetry, document = globalThis.document} = {}) {
  ...
  for (const title of ['Step / source', 'Attempts¹', 'Checkpoint / confirmed / interrupted', 'Repeat / demo requests', 'Tracking loss', 'Time²']) { const th = el('th', title); th.scope = 'col'; headRow.append(th); }
  head.append(headRow); const rows = el('tbody'); table.append(caption, head, rows); tableWrap.append(table);
  const caveat = el('p', '¹ Includes the active attempt; checkpoints are movement only; confirmations are explicit user reports. Demo requests count the explicit Watch demo action only. ² Exclusive observed states; hidden time, reported frame stalls and gaps over 2.5s are unknown. Time is not evidence of confusion or learning quality.', 'td-note');
