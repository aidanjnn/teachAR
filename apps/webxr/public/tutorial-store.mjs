...
function database(){
  return new Promise((resolve,reject)=>{
    const request=indexedDB.open('trail-tutorials',2);
    request.onupgradeneeded=()=>{
      const db=request.result;if(!db.objectStoreNames.contains('drafts'))db.createObjectStore('drafts');
...
export async function saveTutorial(tutorial,expected){
  ...
  try{
    await new Promise((resolve,reject)=>{
      const tx=db.transaction(['drafts','library'],'readwrite'),store=tx.objectStore('drafts');let failure;
      ...
      read.onsuccess=()=>{
        ...
        existing.onsuccess=()=>{
          ...
          store.put(tutorial,'current');library.put(tutorial);
        };
      };
    });
  }finally{db.close();}
