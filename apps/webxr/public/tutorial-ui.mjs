...
export function tutorialView(g){
 ...
 v.voiceActive=!!g.voice?.active;
 const b=(id,label)=>v.buttons.push({id,label});
 switch(g.mode){
 case 'polish':{
  v.tag='CREATE · INSTRUCTION VOICE';v.title=g.polishBusy?'Preparing your instruction…':g.polishDraft?'Does this explain your step?':'Turn your explanation into a guide';
  v.text=g.polishDraft?.instruction||'Remove filler and repetition from this recording. Review the wording, then save a clear AI voice for the learner.';
  v.detail=g.polishBusy?'Your original recording stays intact.':g.polishDraft?(g.polishDraft.needsReview?'Some wording was ambiguous. Check carefully; edit text in browser tools if needed.':'Approval generates AI speech and saves it on this device.'):'Uses paid AI only when requested. Replaying saved speech is local.';
  if(!g.polishBusy){if(g.polishDraft)b('polish-approve','Approve wording & generate voice');else {if(g.player.step.narration)b('polish-draft','Polish recorded explanation');b('polish-written','Use current written instruction');}if(g.player.step.instruction_voice)b('polish-original','Use original recording');}
  b('polish-back',g.polishBusy?'Cancel':'Back to recording');break;}
 case 'voice':v.tag='VOICE';v.title='Keep your hands on the task';v.text=g.voice?.message||'Speak naturally: “save it now”, “go back please”, or “I didn’t get that”.';v.detail=`${g.voice?.attempts||0} / 60 clips · AI voice · Speech sent while on · 10 min limit`;b(g.voice?.active?'voice-stop':'voice-enable',g.voice?.active?'Turn commands off':'Enable voice controls');b('voice-pair','Enter pairing code');b('voice-help','Commands & help');b(g.coach?.active?'coach-stop':'coach-start',g.coach?.active?'Stop voice coach':'Live coach (separate)');if(g.coach?.active)b('coach-ask','Ask coach');b('voice-back','Back to task');break;
 case 'voice-help':{
  const pages=[['Follow, at your pace','“I didn’t get that” replays this step. “Go back please” shows the previous step. “Next step” moves ahead. “Pause” and “continue” control guidance.'],['Record without reaching','“Start recording” starts the countdown. “Save it now” saves a step. “Pause recording” pauses. “Finish tutorial” ends continuous recording.'],['Ask, or take a break','“What do I do?” reads this step. Ask questions about its recorded instructions. “Go home” protects an unfinished take. “Stop listening” turns commands off.']];
  const [title,text]=pages[g.voiceHelpPage||0];v.tag=`VOICE · HELP ${(g.voiceHelpPage||0)+1} / 3`;v.title=title;v.text=text;v.detail='Say one request, then wait for the reply. Voice navigation does not verify the physical result.';b('voice-help-next','More examples');b('voice-settings','Voice settings');b('voice-back','Back to task');break;}
 ...
 case 'settings':v.tag='PREFERENCES';v.title='Make room for the task';v.text='Pinch Move and drag. Hold Rotate and turn your hand to tilt the panel. Hold Resize and pull outward or inward. Face me straightens it toward you.';b('theme',`Appearance: ${g.appearance?.theme==='light'?'Warm gray':'Charcoal'}`);b('sound',`Event sounds: ${g.appearance?.sound?'On':'Off'}`);b('panel-place','Bring panel here');b('panels-reset','Reset panel layout');b('boundary-help','Quest boundary help');b('settings-back','Back');break;
 ...
 case 'review-step':v.tag=`CREATE · REVIEW ${g.player.index+1} / ${g.tutorial.steps.length}`;v.title='Review your recording';v.text=g.player.step.instruction;v.detail=g.player.step.narration_issue?'Narration needs repair. Open Edit recording.':g.player.step.instruction_voice?'AI instruction voice · Original recording retained. Review before approving.':'Choose required hands and inspect the start and ending before approving.';b('primary',g.saveStatus==='failed'?'Retry approval & save':'Approve & save');b('replay','Replay recording');b('review-pause',g.audioPlayer?.node||!g.player.paused?'Pause preview':'Resume preview');b('polish-open','Polish instruction voice');b('hand','Record replacement');b('review-options','Edit recording');b('keep-add','Keep & add next');break;
 ...
 if(g.voice?.active&&!g.mode.startsWith('voice'))v.detail=`Voice ${g.voice.state} · ${g.voice.message}`;
...
const gLabel=view=>view.voiceActive?'Mic on':'Voice';
export function uiButtons(view){
 const buttons=view.buttons.filter(b=>b.id!=='home');
 const global=[{id:'voice-open',label:gLabel(view),x:432,y:20,w:136,h:48},{id:'home',label:'Home',x:584,y:20,w:136,h:48},{id:'settings',label:'Settings',x:736,y:20,w:140,h:48},{id:'exit',label:'Exit AR',x:892,y:20,w:148,h:48}];
 if(['settings','media-wait'].includes(view.mode))global.splice(2,1);
...
export function drawTutorialUI(g,ctx,hover){
 ...
 ctx.fillStyle=p.muted;ctx.font='26px system-ui';g.text(ctx,v.text,40,184,v.compact?610:980,34,v.compact?2:v.mode==='polish'?4:3);
 if(v.progress!==null&&v.progress!==undefined){box(40,289,610,4,p.line,2);box(40,289,Math.max(4,610*v.progress),4,p.ink,2);}
 const detailY=v.mode==='library'?439:v.compact?274:Math.min(316,Math.min(...g.uiButtons.filter(b=>!['settings','exit','home','voice-open'].includes(b.id)).map(b=>b.y))-24);
 ...
 for(const [index,b]of g.uiButtons.entries()){
  const utility=['settings','exit','home','voice-open'].includes(b.id),primary=index===0&&v.mode!=='home'&&!b.card;
