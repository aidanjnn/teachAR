...
export function tutorialView(g){
 ...
 case 'learn':{
  ...
  b('replay',g.watchOnly?(g.player.paused?'Resume replay':'Pause replay'):(g.gatePaused?'Resume':'Pause'));b('restart-follow','Repeat');if(g.coach?.active){const m=g.coach.mode;b('coach-ask',m==='listening'?'Listening…':m==='live'?'Ask coach':m==='connecting'?'Coach connecting…':'Coach: text only');}b('learn-options','Menu');break;}
 ...
 // A fresh coach answer takes the detail line in any learn phase; guidance text returns after twelve seconds.
 if(g.mode==='learn'&&g.coach?.active&&g.coach.caption&&g.coach.captionAgeMs<12000&&!g.gatePaused&&!['tracking','reference-gap','checkpoint'].includes(g.followEngine?.state))v.detail=`Coach: ${g.coach.caption.slice(0,150)}`;
