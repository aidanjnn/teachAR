...
(async()=>{
 ...
 try{
  ...
  const result=await page.evaluate(async()=>{
   ...
   const g=new TutorialGuide({speak:()=>{},exit:()=>{}}),scene=new THREE.Scene();g.attach(scene);await Promise.all([g.skinHand.loaded,g.leftGhost.skinHand.loaded,...Object.values(g.liveHands).map(h=>h.skinHand.loaded)]);g.begin();
   ...
   if(!g.alignmentResult.matched||!g.liveHands.left.ghost.visible||!g.skinHand.root.visible)fail('Holograms/palm comparison unavailable');
