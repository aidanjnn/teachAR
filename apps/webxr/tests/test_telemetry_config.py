...
class TelemetryRouteTests(unittest.TestCase):
    ...
    def test_cross_origin_configuration_request_rejected(self):
        for headers in [{'Origin':'https://example.com'}, {'Sec-Fetch-Site':'cross-site'}, {'Sec-Fetch-Site':'same-site'},
    ...
    def test_telemetry_assets_use_public_allowlist(self):
        with tempfile.TemporaryDirectory() as directory:
            public = Path(directory)/'public'
            (public/'vendor').mkdir(parents=True)
            for name, mime in [('telemetry-sentry.mjs', 'text/javascript'), ('telemetry.css', 'text/css'),
                               ('vendor/sentry.mjs', 'text/javascript'), ('vendor/SENTRY-LICENSE.txt', 'text/plain')]:
                (public/name).write_text('synthetic public asset')
                with patch.object(server, 'ROOT', Path(directory)):
                    with urllib.request.urlopen(self.origin+'/'+name) as response:
                        self.assertEqual(response.headers.get_content_type(), mime)
                        self.assertEqual(response.read(), b'synthetic public asset')
