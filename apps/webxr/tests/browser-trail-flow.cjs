...
(async()=>{const browser=await chromium.launch({channel:process.env.TRAIL_BROWSER_CHANNEL||undefined,headless:true,args:['--enable-unsafe-swiftshader','--mute-audio']});try{
 ...
 await page.goto(`${process.env.TRAIL_TEST_ORIGIN||'http://127.0.0.1:4321'}/`);await page.getByRole('button',{name:'Create tutorial',exact:true}).click();
 ...
 const result=await page.evaluate(async()=>{
  ...
  a.action('create');if(a.mode!=='save-home')fail('Create must begin with save-position setup');
