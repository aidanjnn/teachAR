...
class HTTPTests(unittest.TestCase):
    ...
    def test_product_home_and_legacy_lab_routes(self):
        for path, filename in [('/', 'tutorial.html'), ('/tutorial', 'tutorial.html'), ('/lab', 'index.html')]:
            with self.subTest(path=path), urllib.request.urlopen(self.base+path) as response:
                self.assertEqual(response.status, 200)
                self.assertEqual(response.read(), (server.ROOT/'public'/filename).read_bytes())
