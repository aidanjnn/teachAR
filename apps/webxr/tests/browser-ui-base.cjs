...
(async()=>{const browser=await chromium.launch({headless:true,channel:process.env.TRAIL_BROWSER_CHANNEL||undefined,args:['--enable-unsafe-swiftshader','--mute-audio']});try{
 ...
 await page.route('**/ar.js',async route=>{const response=await route.fetch();await route.fulfill({response,body:(await response.text())+'\nexport {action,guide};'});});
 ...
 const result=await page.evaluate(async()=>{
  ...
  const bad=new TutorialGuide({speak:()=>{},exit:()=>{},onFeedback:e=>events.push(e),writeTutorial:()=>Promise.reject(Error('Disk full'))});bad.attach(new THREE.Scene());bad.tutorial=syntheticTutorial();bad.tutorial.steps.forEach(s=>{s.guide_hands='both';s.reviewed=true;});bad.ux=true;bad.activeSession=true;bad.mode='author';
  ...
  let lateRelease;const late=new TutorialGuide({speak:()=>{},exit:()=>{},onFeedback:e=>events.push(e),writeTutorial:()=>new Promise(r=>lateRelease=r)});late.tutorial=syntheticTutorial();late.tutorial.steps.forEach(s=>{s.guide_hands='both';s.reviewed=true;});late.ux=true;late.mode='author';const pending=late.finishAuthoring();while(!lateRelease)await new Promise(r=>setTimeout(r,0));late.reset();const count=events.length;lateRelease();await pending;if(late.mode==='saved'||events.length!==count)fail('Old session save changed new session UI');
  ...
  g.player.step.guide_hands='recorded';g.action('keep-add');if(g.mode!=='review-step'||g.player.step.reviewed)fail('Keep/add approved automatic hand selection');
  g.player.step.guide_hands='both';const left=g.player.step.frames[10].left;g.player.step.frames[10].left=null;g.action('keep-add');if(g.mode!=='review-step'||g.player.step.reviewed)fail('Keep/add approved a required-hand gap');g.player.step.frames[10].left=left;
