...
(async()=>{const browser=await chromium.launch({headless:true,args:['--enable-unsafe-swiftshader','--mute-audio']});try{
 ...
 await page.evaluate(async()=>{
  ...
  guide.persist=()=>Promise.resolve();guide.begin('home');guide.action('create');guide.action('create-continue');guide.action('media-enable');
  ...
  guide.action('create');guide.action('create-continue');guide.action('media-enable');guide.action('settings');
  ...
  guide.action('create');guide.action('create-continue');guide.action('media-enable');guide.endSession();guide.begin('home');
  finish({camera:true,microphone:true,errors:[]});await guide.mediaTask;
