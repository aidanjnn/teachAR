export function mountShell(root: HTMLElement) {
  root.innerHTML = `
    ...
    <main>
      ...
      <section class="diagnostics" aria-label="Runtime diagnostics"><div><span>Local server</span><strong id="health" role="status">Checking…</strong></div><div><span>Headset app</span><strong>WebXR · Quest Browser</strong></div><div><span>Integrations</span><strong id="providers">Checking…</strong></div><div><span>Vision backend</span><strong id="vision" role="status">Checking…</strong></div><button id="refresh-health" type="button">Recheck server</button></section>
