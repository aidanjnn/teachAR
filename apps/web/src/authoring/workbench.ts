import { RecordingSchema, TutorialSchema, SpectatorStateSchema, type Recording, type Tutorial, type TutorialDraftEdit, type SpectatorState } from '@trail/contracts';
...
export function mountWorkbench(root: HTMLElement) {
  root.innerHTML = `
    ...
    <section id="authoring-panel" hidden aria-label="Tutorial authoring">
      ...
      <div class="author-toolbar"><button id="import-demo" class="primary">Import four-step sample</button><label class="file-button">Import recording <input id="recording-file" type="file" accept=".json,application/json"></label><button id="reload-library">Reload saved guides</button><select id="tutorial-library" aria-label="Saved guides"><option value="">Choose a saved guide</option></select></div>
  ...
  let tutorial: Tutorial | undefined; let recording: Recording | undefined; let draft: TutorialDraftEdit | undefined; let selected = 0; let dirty = false;
  ...
  async function load(id: string) {
    ...
    recording = RecordingSchema.parse(result.recording); draft = editOf(tutorial); selected = 0; dirty = false;
  ...
  async function importRecording(input: unknown) {
    ...
    if (pending) {
      ...
      const created = await api('/api/recordings', { metadata }) as { id: string }; id = created.id;
  ...
  node(root, '#save-review').addEventListener('click', () => { void busy(async () => {
    if (!tutorial || !draft) return; captureFields(); tutorial = TutorialSchema.parse(await api(`/api/tutorials/${tutorial.id}`, draft, 'PATCH')); draft = editOf(tutorial); dirty = false; renderSteps(); await library(); status('Reviewed draft saved. Targets were recalculated from the recording.');
  }); });
  ...
  function renderSpectator() {
    ...
    if (snapshot && 'state' in snapshot) {
      node(root, '#spectator-phase').textContent = snapshot.state.phase.replaceAll('-', ' '); node(root, '#spectator-step').textContent = snapshot.state.stepId ?? 'Run complete';
      ...
      node(root, '#spectator-evidence').textContent = snapshot.state.phase === 'complete' ? 'Movement checkpoint reached. Physical outcome remains learner-confirmed.' : 'Only the headset can advance the guide.';
    }
  }
  function connectSpectator() {
    ...
    socket.onmessage = event => { const parsed = SpectatorStateSchema.safeParse(JSON.parse(String(event.data))); if (parsed.success) { const incoming = parsed.data; const before = spectator?.snapshot; if (incoming.snapshot && before && incoming.snapshot.runId === before.runId && incoming.snapshot.seq < before.seq) return; spectator = incoming; renderSpectator(); } };
  ...
  const dispose = () => { spectatorActive = false; clearInterval(timer); clearTimeout(reconnect); socket?.close(); viewer?.dispose(); };
