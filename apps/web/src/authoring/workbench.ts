...
export function mountWorkbench(root: HTMLElement) {
  root.innerHTML = `
    ...
    <section id="pair-panel" class="pair-panel" hidden aria-label="Pair this browser"><form id="pair-form"><label>Pairing code <input id="pair-code" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{8}" maxlength="8" required placeholder="8 digit code"></label><button type="submit">Connect workspace</button></form><label class="invite-role">New device <select id="invite-role"><option value="learner">Learner</option><option value="spectator">Spectator</option><option value="author">Author</option></select></label><button id="issue-code" type="button">Create pairing code</button><output id="invite-code"></output><p id="pair-state" role="status">Use the private code from the demo operator.</p></section>
  ...
  node(root, '#issue-code').addEventListener('click', () => { void busy(async () => {
    const issued = await api('/api/pairing-codes', { role: node<HTMLSelectElement>(root, '#invite-role').value }) as { code: string; expiresAt: number };
