...
remains simulated. The voice coach is connected: the tutor publishes reviewed step text as a server coach guide, starts the GPT-Live coach from its Voice coach card, and drafts step titles and instructions from narration through Whisper and the label route; it needs the paired main API and, for live voice, `AI_PROVIDER=openai`.md).
...
### Voice coach through the main API

The main API also serves the tutor at `/tutorial`, so one origin covers the page, pairing and the
voice routes: `pnpm dev:desktop` (or `pnpm build && pnpm start:server`) with
`ALLOW_USB_LOOPBACK=true PAIRING_ORIGINS=http://localhost:3001`, then `adb reverse tcp:3001 tcp:3001`
and open `http://localhost:3001/tutorial` in Quest Browser. Pair the page with an author code (the
desktop authoring page mints one; `data/<dir>/pairing.json` holds the bootstrap code), press Start
coach before entering AR, and use Ask coach on the practice panel. `POST /api/coach-guides` stores
the reviewed step text the coach answers from. Microphone, WebRTC and an immersive session together
on the Quest are not yet verified.
