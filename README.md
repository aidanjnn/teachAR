...
The functional browser tutor is packaged in `experiments/quest-browser`.
Start with [the Quest Browser instructions][browser-guide]; the product entry is
`/tutorial` on port 4321. `pnpm dev` starts the separate desktop/backend app.
No Unity install or API key is needed for hand recording and ghost guidance.
The older [Unity handoff](docs/ux-unity-handoff.md) remains available for native work;
private recordings and keys are excluded.
