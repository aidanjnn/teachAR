...
`pnpm dev:desktop` for that stack. Voice/vision adapters are not yet wired end to
end into the tutor. The clean UI is connected to real actions; its standalone
...
- Voice and fresh visual coaching are integration targets, not established tutor
  behavior. Passthrough display is not camera-pixel access. Feature-detect camera,
